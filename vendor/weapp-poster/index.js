  ...
  drawBlock({
    ...
    if (typeof texts !== "undefined") {
      const textWidth = this._getTextWidth(
        typeof texts.text === "string" ? texts : texts.text
      );
      isOverflow = textWidth > width;
      blockWidth = isOverflow && expand ? textWidth : width;
...
const handle = {
  ...
  _drawSingleText({
    ...
    textArr.forEach((item, index) => {
      let fy = this.toPx(y + (height || fontSize) * index);
      if (fontBold) {
        this.ctx.fillText(item, fx, fy);
        this.ctx.fillText(item, fx, fy + 0.5);
        this.ctx.fillText(item, fx + 0.5, fy);
      } else {
        this.ctx.fillText(item, fx, fy);
      }
    });
  ...
  methods: Object.assign(
    {
      ...
      downloadResource() {
        ...
        return new Promise((resolve, reject) => {
          if (
            ...
            if (config.images) {
              this.downloadResourceCanvas(config.images)
                .then(() => {
                  this.downloadStatus = "success";
                  resolve();
                })
      ...
      create(config) {
        ...
        this.initCanvas(config.width, config.height, config.debug || false)
          .then(() => {
